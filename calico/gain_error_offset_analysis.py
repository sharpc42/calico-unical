...
def main(calibrate : bool = True, 
    ...
    if calibrate:
        ...
        for sigma_t in sigma_t_scales:
            ...
            for sigma_m in sigma_m_scales:
                ...
                __import__('many_realizations_study').init_many_realizations(
                    ...
                    run_params_filename          = f'gain_error_offset_analysis_{suffix}_run_params',
                ...
                os.system(f"rm {data_path}/gain_error_offset_analysis_{suffix}_run_params.pkl")
                os.system(f"rm {data_path}/gain_error_offset_analysis_output_calcs.pkl")
                
        if verbose: 
            print("Writing out run-aggregate files...")
        with open(f'{data_path}/gain_error_offset_analysis_vT_minus_m.pkl', 'wb') as file:
            pickle.dump(vT_minus_m_gaussian, file)
        with open(f'{data_path}/gain_error_offset_analysis_real_sigma_t_calculated_gaussian.pkl', 'wb') as file:
            pickle.dump(real_sigma_t_calculated_gaussian, file)
        with open(f'{data_path}/gain_error_offset_analysis_real_sigma_uvT_truth_gaussian.pkl', 'wb') as file:
            pickle.dump(real_sigma_uvT_truth_gaussian, file)
        with open(f'{data_path}/.pkl', 'wb') as file:
            pickle.dump(real_sigma_uvT_skycal_gaussian, file)
        with open(f'{data_path}/gain_error_offset_analysis_real_g_minus_1_truth_gaussian', 'wb') as file:
            pickle.dump(real_g_minus_1_truth_gaussian, file)
        with open(f'{data_path}/gain_error_offset_analysis_real_g_minus_1_skycal_gaussian', 'wb') as file:
            pickle.dump(real_g_minus_1_skycal_gaussian, file)
        if verbose: 
            print("Done.")
        ...
        with open(f'{data_path}/gain_error_offset_analysis_vT_minus_m.pkl', 'rb') as file:
            vT_minus_m_gaussian = pickle.load(file)
        with open(f'{data_path}/gain_error_offset_analysis_real_sigma_t_calculated_gaussian.pkl', 'rb') as file:
            real_sigma_t_calculated_gaussian = pickle.load(file)
        with open(f'{data_path}/gain_error_offset_analysis_real_sigma_uvT_truth_gaussian.pkl', 'rb') as file:
            real_sigma_uvT_truth_gaussian = pickle.load(file)
        with open(f'{data_path}/gain_error_offset_analysis_real_sigma_uvT_skycal_gaussian.pkl', 'rb') as file:
            real_sigma_uvT_skycal_gaussian = pickle.load(file)
        with open(f'{data_path}/gain_error_offset_analysis_real_g_minus_1_truth_gaussian', 'rb') as file:
            real_g_minus_1_truth_gaussian = pickle.load(file)
        with open(f'{data_path}/gain_error_offset_analysis_real_g_minus_1_skycal_gaussian', 'rb') as file:
            real_g_minus_1_skycal_gaussian = pickle.load(file)

    get_git_suffix = str(subprocess.check_output(['git','rev-parse','--short','HEAD']).decode('ascii').strip())
    ...
    dev.plot_3d_data_as_2d_hist(
        ...
        plot_vmax   = 0.25,
        plot_vmin   = -0.1,
        filename    = f'{image_path}/{filename_2d_gains}_truth_{get_git_suffix}_gaussian.png',
    )
    if verbose:
    ...
    dev.plot_3d_data_as_2d_hist(
        ...
        plot_vmax   = 0.25,
        plot_vmin   = -0.1,
        filename    = f'{image_path}/{filename_2d_gains}_skycal_{get_git_suffix}_gaussian.png',
    )
    if verbose:
    ...
    dev.plot_3d_data_as_2d_hist(
        ...
        plot_ylabel = "$\\sigma_t (Re)$",
        log_cmap    = True,
        filename    = f'{image_path}/{filename_2d_gains}_diff_{get_git_suffix}_gaussian.png',
    )
    if verbose:
    ...
    dev.plot_3d_data_as_2d_hist(
        ...
        plot_vmax   = 15,
        plot_vmin   = -5,
        filename    = f'{image_path}/{filename_2d_u_err}_truth_{get_git_suffix}_gaussian.png',
        plot_cmap   = "inferno",
    )
    if verbose:
    ...
    dev.plot_3d_data_as_2d_hist(
        ...
        plot_vmax   = 15,
        plot_vmin   = -5,
        filename    = f'{image_path}/{filename_2d_u_err}_skycal_{get_git_suffix}_gaussian.png',
        plot_cmap   = "inferno",
    )
    if verbose:
    ...
    dev.plot_3d_data_as_2d_hist(
        ...
        plot_vmax   = 15,
        plot_vmin   = -5,
        filename    = f'{image_path}/{filename_2d_u_err}_diff_{get_git_suffix}_gaussian.png',
        plot_cmap   = "inferno",
    )
    if verbose:
    ...
    dev.build_3d_scatter_plot(
        ...
        zlim_hi     = 1,  zlim_lo     = -1,
        filename    = f'{image_path}/{filename_3d_scatter}_{get_git_suffix}_gaussian.png',
    )
